function numberFromElementText(aElement) {
  return typeof aElement == "undefined" ? 0 : 
...
class Resource extends Serializable {	
	...
	static fromTR(prov, aTr) {
	  ...
	  keys.forEach(function(key, val){
	  	if(key != null){
	  		values[key] = numberFromElementText(tds[val + 1]);
	  	}
	  });
...
VorschauTable = class VorschauTable {
	...
	constructor(srcTable, aProvName, modifyTable){
	  ...
	  for (i = 0; i < this.body.rows.length; i++) {
	    var current = this.body.rows[i];
	    var res = Resource.fromTR(this.provName, current);
